...
export function NewsCard({ item, size, index, accentFrom }: Props) {
  ...
  const snippet = item.kind === "news" ? item.snippet : item.artist;
  const source = item.kind === "news" ? item.source : "Spotify";
  const meta =
    item.kind === "news"
      ? formatDate(item.publishedAt)
      : `${formatReleaseLabel(item.albumType)} · ${formatDate(item.releaseDate)}`;
    ...
    >
      <div className={`absolute inset-0 ${SIZE_ASPECT[size]} md:h-full md:w-full`}>
        <ImageWithFallback
          ...
          fallbackGradient={
            item.kind === "news" && item.fallbackGradient
              ? item.fallbackGradient
              : `linear-gradient(135deg, ${accentFrom}, oklch(0.18 0.1 280))`
          }
        />
      ...
      <div className="relative flex h-full flex-col justify-end p-5 md:p-6">
        ...
        {snippet && (size === "hero" || size === "banner" || size === "wide") && (
